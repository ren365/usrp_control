function [txWaveform,afterMap] = wlanWaveformGenerator_Debug(dataBits,cfgFormat,varargin)
...
%   <a href="matlab:help('wlanHTConfig')">wlanHTConfig</a>, <a href="matlab:help('wlanNonHTConfig')">wlanNonHTConfig</a>, or <a href="matlab:help('wlanWURConfig')">wlanWURConfig</a>. The format of the
%   generated waveform is determined by the type of CFGFORMAT. The
...
%                              for HE, WUR, S1G, VHT, HT, and non-HT OFDM
%                              formats. The oversampling factor must be
...
%   wlanWURConfig, wirelessWaveformGenerator.
...
validateattributes(cfgFormat,{'wlanVHTConfig','wlanHTConfig','wlanNonHTConfig','wlanS1GConfig','wlanDMGConfig','wlanHESUConfig','wlanHEMUConfig','wlanHETBConfig','wlanWURConfig'},{'scalar'},mfilename,'format configuration object');
...
isOFDM = isHE || isDMGOFDM || isS1G || isVHT || isHT || isWUR || ...
...
if nargin==2
    ...
    scramValFn = @(x)validateScramblerInitialization(x,inDSSSMode,isDMG,isNonHT,isWUR,isOFDM,cfgFormat);
    winValFn = @(x)validateWindowTransitionTime(x,isOFDM,isHE,isDMG,isS1G,isNonHT,cfgFormat);
    ...
    for i = 1:useParams.NumPackets
        ...
        elseif isNonHT
            if strcmp(cfgFormat.Modulation, 'OFDM')
                [data,afterMap] = wlanNonHTData_Debug(psdu{1},cfgFormat,pktScramInit(i,:),osfNV{:});
            else % DSSS
...
function validateWindowTransitionTime(tt,isOFDM,isHE,isDMG,isS1G,isNonHT,cfgFormat)
