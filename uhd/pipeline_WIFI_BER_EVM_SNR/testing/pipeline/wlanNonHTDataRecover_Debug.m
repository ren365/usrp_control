function [bits, eqDataSym, varargout,beforeDemap] = wlanNonHTDataRecover_Debug( ...
...
validateattributes(rx, {'double'}, {'2d','finite'}, 'rxHTData', 'Non-HT OFDM Data field signal');
% Validate chanEst
...
coder.internal.errorIf(size(chanEst, 1) ~= ofdmInfo.NumTones, 'wlan:wlanNonHTDataRecover:InvalidNHTChanEst1D', ofdmInfo.NumTones);
...
ofdmDataOutOne20MHz = coder.nullcopy(complex(zeros(NsdSeg, numOFDMSym, numRx*ofdmInfo.NumSubchannels))); % Preallocate for codegen
chanEstDataOne20MHz = coder.nullcopy(complex(zeros(NsdSeg, 1, numRx*ofdmInfo.NumSubchannels))); % Preallocate for codegen
...
% Demap and decode
[bits,scraminit,beforeDemap] = wlanNonHTDataBitRecover_Debug(eqDataSym,noiseVarEst,csiData,cfgNonHT);
varargout{2} = scraminit;
